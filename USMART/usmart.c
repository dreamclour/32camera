...
u8 usmart_sys_cmd_exe(u8 *str)
{
	...
	case 1: // 帮助指令
		RTT_printf(0,"\r\n");
#if USMART_USE_HELP
		RTT_printf(0,"------------------------USMART V3.1------------------------ \r\n");
//		RTT_printf(0,"    USMART是由ALIENTEK开发的一个灵巧的串口调试互交组件,通过 \r\n");
//		RTT_printf(0,"它,你可以通过串口助手调用程序里面的任何函数,并执行.因此,你可\r\n");
//		RTT_printf(0,"以随意更改函数的输入参数(支持数字(10/16进制)、字符串、函数入\r\n");
//		RTT_printf(0,"口地址等作为参数),单个函数最多支持10个输入参数,并支持函数返 \r\n");
//		RTT_printf(0,"回值显示.新增参数显示进制设置功能,新增进制转换功能.\r\n");
//		RTT_printf(0,"技术支持:www.openedv.com\r\n");
//		RTT_printf(0,"USMART有7个系统命令:\r\n");
//		RTT_printf(0,"?:      获取帮助信息\r\n");
//		RTT_printf(0,"help:   获取帮助信息\r\n");
//		RTT_printf(0,"list:   可用的函数列表\r\n\n");
//		RTT_printf(0,"id:     可用函数的ID列表\r\n\n");
//		RTT_printf(0,"hex:    参数16进制显示,后跟空格+数字即执行进制转换\r\n\n");
//		RTT_printf(0,"dec:    参数10进制显示,后跟空格+数字即执行进制转换\r\n\n");
//		RTT_printf(0,"runtime:1,开启函数运行计时;0,关闭函数运行计时;\r\n\n");
//		RTT_printf(0,"请按照程序编写格式输入函数名及参数并以回车键结束.\r\n");
		// RTT_printf(0,"    USMART is a clever serial debugging interinterface developed by ALIENTEK.\r\n");
		// RTT_printf(0,"Through it, you can call any function in the program through the serial Assistant\r\n");
		// RTT_printf(0," and execute it. Therefore, you can change the input parameters of the function\r\n");
		// RTT_printf(0," (support numbers (10/16), strings, function entry address, etc.), a single function\r\n");
		// RTT_printf(0," supports up to 10 input parameters, and support function return value display.\r\n\n");
		//RTT_printf(0,"technical support:www.openedv.com\r\n\n");
		RTT_printf(0,"USMART has 7 system commands:\r\n");
		RTT_printf(0,"?:      Get Help\r\n");
		RTT_printf(0,"help:   Get Help\r\n");
		RTT_printf(0,"list:   List of available functions\r\n");
		RTT_printf(0,"id:     List of available functions ID\r\n");
		RTT_printf(0,"hex:    Parameter hexadecimal display, \r\n");
		RTT_printf(0,"        instruction followed by a space + digits to perform a decimal conversion\r\n");
		RTT_printf(0,"dec:    Parameter decimal display, \r\n");
		RTT_printf(0,"        instruction followed by a space + digits to perform a decimal conversion\r\n");
		RTT_printf(0,"runtime 1/0: 1, Enable function runtime; 0, turn off the function runtime\r\n\n");
		RTT_printf(0,"Enter the function name and parameters as programmed and end with the Enter key.\r\n");
		RTT_printf(0,"--------------------------ALIENTEK------------------------- \r\n");
#else
		RTT_printf(0,"指令失效\r\n");
#endif
		break;
	...
	case 4: // hex指令
		...
		if (i == 0) // 参数正常
		{
			...
			if (i == 0)						  // 进制转换功能
			{
				RTT_printf(0,"HEX:0X%X\r\n", res); // 转为16进制
			}
			else if (i != 4)
				return USMART_PARMERR; // 参数错误.
			else					   // 参数显示设定功能
			{
				RTT_printf(0,"Display HEX!\r\n");
			...
			if (i == 0)						  // 读取指定地址数据功能
			{
				...
				else
				{
					...
					if (usmart_dev.runtimeflag)
						RTT_printf(0,"Run Time Calculation ON\r\n");
					else
						RTT_printf(0,"Run Time Calculation OFF\r\n");
				}
			}
			else
...
void TIM4_IRQHandler(void)
{
	...
	{
		//读取RTT数据//////////////////////////////////////////////////////////////////////
		if(SEGGER_RTT_HasKey() != 0)    //识别缓存中有可打印字符
		{
            //char buf = 0;
			int ret = 0;
            int i = 0;

            ret = SEGGER_RTT_Read(0, USART_RX_BUF+ret, 255);
...
void usmart_exe(void)
{
	...
	for (i = 0; i < pnum; i++)												// 输出参数
	{
		...
		{
			RTT_printf(0,"%c", '"');
			RTT_printf(0,"%s", usmart_dev.parm + usmart_get_parmpos(i));
			RTT_printf(0,"%c", '"');
		...
		else // 参数是数字
		{
			...
			if (usmart_dev.sptype == SP_TYPE_DEC)
				RTT_printf(0,"%lu", temp[i]); // 10进制参数显示
			else
				RTT_printf(0,"0X%X", temp[i]); // 16进制参数显示
		}
		if (i != pnum - 1)
			RTT_printf(0,",");
	}
	...
	if (USART_RX_STA & 0x8000) // 串口接收完成？
	{
		...
		else
		{
			...
			if (sta)
			{
				...
				{
				case USMART_FUNCERR:
					RTT_printf(0,"Function error!\r\n");
					break;
				case USMART_PARMERR:
					RTT_printf(0,"Parameter error!\r\n");
					break;
				case USMART_PARMOVER:
					RTT_printf(0,"Too many parameters!\r\n");
					break;
				case USMART_NOFUNCFIND:
					RTT_printf(0,"Function not found!\r\n");
					break;
				}
			}
		}
